    ...
    return (
        <Box maxW="md" borderWidth="1px" borderRadius="lg" overflow="hidden" boxShadow="md" p={4} mb={4}>
            <Box display="flex" alignItems="center" mb={2}>
                ...
                <Link href={`/profile/${publication.by.handle?.localName}.${publication.by.handle?.namespace}`} fontWeight="bold">{publication.by.metadata?.displayName || publication.by.handle.fullHandle}</Link>
            </Box>
            <Link href={`/post/${publication.id}`}>
                ...
                <Text color="gray.500" fontSize="sm">{publication.stats.comments} comments - {dateObj.toLocaleDateString('en-US', options)}</Text>
            </Link>
          </Box>
    )
}